...
def register(request):
    ...
    if request.method == "POST":
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            profile = profile_form.save(commit=False) #commit=False disables saving to the database
            profile.user = user
...
def user_login(request):
    ...
    if request.method == 'POST':
        username = request.POST.get('username')  #the argument passed in get() method should be equal to name parameter filled in the HTML input tag
